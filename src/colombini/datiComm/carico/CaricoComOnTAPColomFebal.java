...
import colombini.query.datiComm.carico.QueryProdComFromTAPColomFebal;
 ...
 * @author lvita
 */
public abstract class CaricoComOnTAPColomFebal implements IDatiCaricoLineaComm {
  ...
  private List<CaricoCommLineaBean> getDatiCommDett(LineaLavBean ll, Connection con) throws DatiCommLineeException {
    ...
    commesse.add(ll.getCommessa());
    String CommFebalAux=Long.toString(ll.getDataCommessa());
    String commFebal=CommFebalAux.substring(0, 4)+ll.getCommessa();
    commesse.add(Integer.parseInt(commFebal));
    try{
      QueryProdComFromTAPColomFebal q=new QueryProdComFromTAPColomFebal();
      q.setFilter(FilterFieldCostantXDtProd.FT_AZIENDA, FilterFieldCostantXDtProd.AZCOLOMBINI);
      q.setFilter(FilterFieldCostantXDtProd.FT_NUMCOMM, commesse.toString());
  ...
  private List<CaricoCommLineaBean> getDatiCommDivi(LineaLavBean ll, Connection con) throws DatiCommLineeException {
   ...
   private static final Logger _logger = Logger.getLogger(CaricoComOnTAPColomFebal.class);   
}
