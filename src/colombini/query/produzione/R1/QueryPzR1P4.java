...
public class QueryPzR1P4 extends CustomQuery {
  ...
  public String toSQLString() throws QueryException {
    ...
    sql.append(" select Collo , ncollo , b.LineaDestAbbreviata , Box, Pedana , odv , rigaOrdine ,  codSemilavorato , descAbb ,  Descrizione  , barcode ").append(
      ...
      if(isFilterPresent(FT_LSM_CUCINE)){
                sql2.append("select * from OPENQUERY(AS400,'select trim(pselno), 1 n_collo,''P2MF1'' linea, clboxn, clpeda, psridn, 0, ''0'', left(cldesc , 25) concat '' '' concat sum(int(pscnqt)) desc1,cldesc concat '' QTA: '' concat sum(int(pscnqt)) desc2,trim(psproj) concat trim(pselno) barcode \n").append(
